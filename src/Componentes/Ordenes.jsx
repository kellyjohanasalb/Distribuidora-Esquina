...
const OrdersView = () => {
    ...
    const ordenesPendientes = todasOrdenes.filter(o =>
        (o.status?.toLowerCase() === 'pendiente' || o.status?.toLowerCase() === 'pending')
    );
    const ordenesEnviadas = todasOrdenes.filter(o =>
        (o.status?.toLowerCase() === 'enviado' || o.status?.toLowerCase() === 'sent')
    );
            ...
            <div className="container-fluid px-2 px-md-4 py-3">
                <div className="row justify-content-center">
                    <div className="col-12">
                        <div className="card shadow-lg border-0" style={{ backgroundColor: '#f7dc6f', borderRadius: '0', minHeight: '100vh' }}>
                            
                            {/* HEADER */}
                            <div className="card-header py-2 py-md-3" style={{ backgroundColor: '#f7dc6f', borderRadius: '15px 15px 0 0' }}>
                                <div className="row align-items-center">
                                    <div className="col-auto">
                                        <img src="/logo-distruidora/logo.png" alt="Distribuidora Esquina" className="logo-img d-none d-md-block" style={{ maxHeight: '120px' }} />
                                        <img src="/logo-distruidora/logo.png" alt="Distribuidora Esquina" className="d-md-none" style={{ maxHeight: '50px', borderRadius: '8px' }} />
                                    </div>
                                    ...
                                    <div className="col">
                                        ...
                                        <div className="row mt-2">
                                            <div className="col-auto">
                                                <span className="badge bg-warning text-dark me-2">Pendientes: {ordenesPendientes.length}</span>
                                            </div>
                                            <div className="col-auto">
                                                <span className="badge bg-success">Enviadas: {ordenesEnviadas.length}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-auto">
                                        <div className="d-flex align-items-center flex-nowrap">
                                            ...
                                            <div className="d-flex align-items-center rounded-pill px-2 py-1 me-2" style={{ backgroundColor: '#298143' }}>
                                                <span className="me-1" style={{ fontSize: '0.9rem' }}>👤</span>
                                                <small className="fw-semibold text-white" style={{ fontSize: '0.7rem' }}>Cliente</small>
                                            </div>
                            ...
                            <div className="card-body p-2 p-md-3 pb-5" style={{ paddingBottom: '120px' }}>
                                ...
                                {/* ALERTAS */}
                                {alert.show && (
                                    <div className={`alert alert-${alert.type === 'success' ? 'success' : alert.type === 'error' ? 'danger' : 'warning'} alert-dismissible fade show d-flex`} role="alert" style={{ maxWidth: '100%' }}>
                                        <div className="d-flex align-items-center flex-grow-1">
                                            {alert.type === 'success' && <Check className="me-2" size={20} />}
                                            {alert.type === 'error' && <X className="me-2" size={20} />}
                                            <div className="text-break" style={{ maxWidth: '85%' }}>{alert.message}</div>
                                        </div>
                                        <button type="button" className="btn-close ms-2" onClick={dismissAlert} aria-label="Cerrar"></button>
                                    </div>
                                )}
                                    ...
                                    <div className="card-body p-3">
                                        <div className="d-flex flex-wrap align-items-center justify-content-between">
                                            ...
                                            <div className="d-flex flex-wrap gap-2">
                                                <button className={`btn btn-sm ${filterType === 'today' ? 'btn-filtro-activo' : 'btn-filtro'}`} onClick={() => handleFilterChange('today')}>📅Hoy</button>
                                                <button className={`btn btn-sm ${filterType === 'all' ? 'btn-filtro-activo' : 'btn-filtro'}`} onClick={() => handleFilterChange('all')}>📋Todos</button>
                                            </div>
                                        </div>
                                        ...
                                        {showDateFilter && (
                                            <div className="mt-3 pt-3 border-top">
                                                <div className="row align-items-center">
                                                    ...
                                                    <div className="col-auto">
                                                        <input type="date" className="form-control form-control-sm input-fecha" value={selectedDate} onChange={(e) => {
                                                    ...
                                                    <div className="col-auto">
                                                        <button className="btn btn-success btn-sm" onClick={() => {
                                                            ...
                                                            <Filter size={14} className="me-1" />Aplicar
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div className="d-flex flex-wrap justify-content-between align-items-center mb-3">
                                    ...
                                    {ordenesPendientes.length > 0 && (
                                        <button className="btn btn-success px-3 py-1" onClick={handleSendAllPending} disabled={isLoading || !isConnected} style={{ fontSize: '0.875rem' }}>
                                            {isLoading ? (
                                                <>
                                ...
                                <div className="d-block d-md-none">
                                    {todasOrdenes.length > 0 ? (
                                        todasOrdenes.map((order) => (
                                            <div key={order.id} className={`card mb-3 ${order.status === 'Pendiente' ? 'border-warning' : 'border-success'}`} style={{ borderRadius: '12px' }}>
                                                <div className="card-body p-3">
                                                    <div className="d-flex justify-content-between align-items-start mb-2">
                                                        <div>
                                                            <h6 className="mb-0 fw-bold text-primary">#{order.id}</h6>
                                                            {order.fechaAlta && (
                                                                <small className="text-muted">📅{formatDate(order.fechaAlta)}</small>
                                                            )}
                                                        </div>
                                                        <span className={`badge px-2 py-1 ${order.status === 'Pendiente' ? 'bg-warning text-dark' : 'bg-success'}`}>{order.status}</span>
                                                    </div>

                                                    <div className="mb-2">
                                                        <div className="text-muted small">Cliente:</div>
                                                        <div className={`fw-semibold ${order.status === 'Enviado' ? 'text-primary' : ''}`} style={{ 
                                                            cursor: order.status === 'Enviado' ? 'pointer' : 'default',
                                                            textDecoration: order.status === 'Enviado' ? 'underline' : 'none'
                                                        }} onClick={() => order.status === 'Enviado' && handleViewOrderDetails(order)} title={order.status === 'Enviado' ? 'Ver detalles del pedido' : ''}>
                                                            {order.name}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="d-grid gap-2">
                                                        {order.status === 'Pendiente' ? (
                                                            <button className="btn btn-success btn-sm" onClick={() => handleSendOrder(order)} disabled={isLoading || !isConnected}>
                                                                <Send size={16} className="me-1" />Enviar
                                                            </button>
                                                        ) : (
                                                            <button className="btn btn-outline-primary btn-sm" onClick={() => handleViewOrderDetails(order)}>
                                                                <Package size={16} className="me-1" />Ver Detalles
                                                            </button>
                                                        )}
                                                    </div>
                                                </div>
                                        ...
                                        !loading && (
                                            <div className="text-center py-5">
                                                ...
                                                <p className="text-muted">
                                                    ...
                                                    {filterType === 'date' && (
                                                        <div className="mt-2">
                                                            <small className="text-info">💡Nota: Se muestran tanto pedidos enviados como pendientes para esta fecha.</small>
                                                        </div>
                                                    )}
                                                </p>
                                                <Link to="/pedido" className="btn btn-success">Crear nuevo pedido</Link>
                                            </div>
                                        )
                                    )}
                                </div>
                                    ...
                                    {todasOrdenes.length > 0 ? (
                                                ...
                                                <tbody>
                                                    {todasOrdenes.map((order) => (
                                                        <tr key={order.id} className={order.status === 'Enviado' ? 'table-success' : ''}>
                                                            ...
                                                            <td>
                                                                <span className={`${order.status === 'Enviado' ? 'text-primary' : ''}`} style={{ 
                                                                    cursor: order.status === 'Enviado' ? 'pointer' : 'default',
                                                                    textDecoration: order.status === 'Enviado' ? 'underline' : 'none'
                                                                }} onClick={() => order.status === 'Enviado' && handleViewOrderDetails(order)} title={order.status === 'Enviado' ? 'Ver detalles del pedido' : ''}>
                                                                    {order.name}
                                                            ...
                                                            <td>
                                                                {order.status === 'Pendiente' && (
                                                                    <button className="btn btn-success btn-sm px-3" onClick={() => handleSendOrder(order)} disabled={isLoading || !isConnected}>
                                                                        <Send size={16} className="me-1" />Enviar
                                                                    </button>
                                                                )}
                                                            </td>
                                        ...
                                        !loading && (
                                            <div className="text-center py-5">
                                                ...
                                                <h5 className="text-muted">No hay órdenes para mostrar</h5>
                                                <p className="text-muted">
                                                    {filterType === 'today' ? 'No hay pedidos para el día de hoy.' : filterType === 'date' ? `No hay pedidos para la fecha ${formatDate(selectedDate)}.` : 'No hay pedidos disponibles.'}
                                                    {filterType === 'date' && (
                                                        <div className="mt-2">
                                                            <small className="text-info">💡Sugerencia: Verifica que la fecha esté en el formato correcto(AAAA - MM - DD)</small>
                                                        </div>
                                                    )}
                                                </p>
                                                <Link to="/pedido" className="btn btn-success">Crear nuevo pedido</Link>
                ...
                <div className="modal-dialog modal-dialog-centered modal-lg">
                    <div className="modal-content">
                        <div className="modal-header">
                            ...
                            <button type="button" className="btn-close" onClick={closeDetailsModal} disabled={loadingDetails}></button>
                        </div>
                                        ...
                                        <div className="card bg-light">
                                            <div className="card-body">
                                                ...
                                                <div className="row">
                                                    ...
                                                    <div className="col-md-6 mb-2">
                                                        <strong>Fecha:</strong>
                                                        <div>{selectedOrderDetails.fechaAlta ? formatDate(selectedOrderDetails.fechaAlta) : 'Sin fecha'}</div>
                                                    </div>
                                                    <div className="col-md-6 mb-2">
                                                        <strong>Total:</strong>
                                                        <div className="text-success fw-bold">{formatCurrency(selectedOrderDetails.total)}</div>
                                                    </div>
                                                    {selectedOrderDetails.observation && selectedOrderDetails.observation.trim() !== "" && selectedOrderDetails.observation.trim() !== "Sin observaciones" && (
                                                        <div className="col-12 mt-2">
                                                            <strong>Observaciones Generales:</strong>
                                                            <div className="bg-white p-2 rounded border">{selectedOrderDetails.observation}</div>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                                ...
                                                <tbody>
                                                    {(selectedOrderDetails.productos || []).map((product, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td className="text-success">{formatCurrency((product.cantidad || 0) * parseFloat(product.precio || 0))}</td>
                                                            <td>
                                                                {product.observation && product.observation.trim() !== "" ? (
                                                                    <small className="text-muted">{product.observation}</small>
                                                                ) : (
                        ...
                        <div className="modal-footer">
                            <button type="button" className="btn btn-secondary" onClick={closeDetailsModal} disabled={loadingDetails}>Cerrar</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className={`modal fade ${showModal ? 'show' : ''}`} style={{ display: showModal ? 'block' : 'none' }} tabIndex="-1">
                <div className="modal-dialog modal-dialog-centered modal-sm">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer">
                            <button type="button" className="btn btn-secondary btn-sm" onClick={() => setShowModal(false)} disabled={isLoading}>Cancelar</button>
                            <button type="button" className="btn btn-success btn-sm" onClick={confirmSendOrder} disabled={isLoading}>
                                {isLoading ? (
                                    <>
                                    ...
                                    <>
                                        <Send size={14} className="me-1" />Confirmar
                                    </>
                                )}
                            </button>
                        </div>
