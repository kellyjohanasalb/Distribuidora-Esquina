...
// Función normalizarFecha mejorada para manejar zonas horarias
const normalizarFecha = (fecha) => {
  ...
  // Si es una fecha en formato YYYY-MM-DD, convertir a ISO con mediodía UTC
...
const mapearEnviado = (pedido) => ({
  ...
  products: (pedido.products || pedido.productos || []).map(producto => ({
    ...producto,
    observation: producto.observation || producto.observacion || null
  }))
});
    ...
    try {
      ...
      setOrdenes(prev => prev.map(o =>
        Number(o.id) === Number(orden.id) ? {
          ...
          fechaAlta: normalizarFecha(response.data.fechaPedido || response.data.fechaAlta)
        } : o
      ));
